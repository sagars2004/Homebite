  ...
  .handler(async ({ data }) => {
    try {
      ...
      const { output: recommendation } = await generateText({
        ...
        prompt: `I have these ingredients: ${data.ingredients.join(", ")}. I want ${data.vibe.toLowerCase()} and I have ${data.time.toLowerCase()}. Tell me exactly what to make tonight. Keep missing ingredients to no more than 3 normal pantry staples.`,
        output: Output.object({ schema: RecommendationSchema }),
      });
      if (!recommendation) throw new Error("No recommendation returned");
      ...
      if (meal) {
        ...
        const { output: rewritten } = await generateText({
          model,
          prompt: `Rewrite these recipe instructions as warm, conversational steps for a home cook in their 20s. Keep every action accurate. Reassure them on tricky moments and say what to look for, not just what to do. Never use delicious, amazing, simply, or easy.\n\n${rawInstructions}`,
          output: Output.object({ schema: z.object({ steps: z.array(z.string()) }) }),
        });
        if (!rewritten) throw new Error("No rewritten steps returned");
        return {
      ...
      const { output: generated } = await generateText({
        model,
        prompt: `Generate a complete, accurate recipe for ${recommendation.dish_name}, suitable for a home cook. Use a warm, direct, conversational tone. Reassure tricky moments and describe what to look for. Never use delicious, amazing, simply, or easy.`,
        output: Output.object({ schema: GeneratedRecipeSchema }),
      });
      if (!generated) throw new Error("No recipe returned");
      return {
        dishName: recommendation.dish_name,
        reason: recommendation.reason,
        timeEstimate: recommendation.time_estimate,
