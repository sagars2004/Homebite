...
export const Route = createFileRoute("/recipe")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Your one confident dinner plan, with ingredients and conversational steps.",
      },
        ...
        <article className="-mt-3 pt-8">
            ...
            <ol className="mt-7 space-y-8">
              {recipe.steps.map((step, index) => (
                ...
                >
                  ...
                  <p className="text-lg leading-[1.7]">{step}</p>
                </li>
              ))}
            </ol>
          </section>
          <div className="mt-12 border-t border-border pt-7">
            <Button
              onClick={reroll}
              variant="outline"
              className="h-13 w-full rounded-xl bg-card text-base"
            >
              <RotateCcw />
              Make something else
            </Button>
  ...
  return (
    <section className="mt-10">
      ...
      <ul className="mt-4 divide-y divide-border border-y border-border">
        {items.length ? (
          items.map((item) => (
            <li key={item} className="flex min-h-12 items-center py-3 text-base">
              {item}
            </li>
          ))
        ) : (
